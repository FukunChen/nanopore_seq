...
################################################################
# 使用parasail在read seq中找到相应的u的位置，并制作index
###############################################################
...
def create_bed(ref_fa_path, bam_path, output_dir, filename):
    ...
    for aln in bam.fetch(until_eof=True):       
        ...
        result = LOCAL_ALIGN_FUNCTION(, ref_seq, GAP_OPEN_PENALTY, GAP_EXTEND_PENALTY, MATRIX)
        ref = result.ref
        query = result.query
        ...
        for i, base in enumerate(ref):
            ...
            #breakpoint()
            # 统计 query 中 [0:i] 之间的非 '-' 数量
            pos_start = sum(1 for q in query[:i] if q != '-')
            # end 就是包括 i 的非 '-' 数量
            pos_end = sum(1 for q in query[:i+1] if q != '-')
            context_ref  = base
